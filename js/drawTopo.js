...
$(function(){
    ...
    //加载已有数据
    topocontent.load('http://www.zhoushuanglong.cn/drag-topo/json/connection.txt', function(response, status){
        if(status == "success"){
            $.closePop('loadingPop');
            editData(response);
        }else{
    ...
    topoView.load('http://www.zhoushuanglong.cn/drag-topo/json/connection.txt', function(response, status){
        if(status == "success"){
            $.closePop('loadingPop');
            viewPop(response);
        }else{
